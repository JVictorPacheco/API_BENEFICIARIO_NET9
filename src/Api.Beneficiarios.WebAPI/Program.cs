...
var connectionString = Evironment.GetEnvironmentVariable("DB_CONNECTION_STRING")  // Serve para obter a string de conexão do banco de dados a partir de uma variável de ambiente
...
{
    throw new InvalidOperationExcetion("Connection string não configurada! Verifique appsettings.json ou variável de ambiente DATABASE_CONNECTION_STRING"); // Lança uma exceção se a string de conexão não estiver configurada corretamente
}
...
builder.Services.AddDbContext<BeneficiariosDbContext>(options =>
{
    ...
    if (builder.Evironment.IsDevelopment())
    {
        options.EnableSensitiveDataLogging(); // Mostra dados sensíveis no log (útil para debug)
        options.EnableDetailsErrors(); // Mostra detalhes dos erros do EF Core
    }
});
...
builder.Service.AddConrollers()
    .AddJsonOptions(options =>
    {
        // Retorna enums como string ("Ativo") ao invés de número (1)
        options.JsonSeralizerOptions.Converters.Add(
            new System.Text.Json.Serialization.JsonStringEnumConverter()
        );

        // Ignora propriedades null no JSON de resposta (deixa JSON mais limpo)
        options.JsonSerializarOptions.DefaultIgnoredCondition =
            System.Text.Json.Serialization.JsonIgnoredCondition.WhenWritingNull;
    });
...
app.UserHttpsRedirection(); // Redireciona requisições HTTP para HTTPS
...
app.Logger.LoginInformation("🚀 Api beneficiarios iniciada!"); // Registra uma mensagem de log informando que a aplicação foi iniciada com sucesso
app.Logger.LoginInformation($"📊 Ambiente: {app.Evironment.EvironmentName}"); // Registra o ambiente atual (Desenvolvimento, Produção, etc.)
app.Logger.LoginInformation($"🗄️ Banco de dados{(connectionString.Contains("localhost") ? "local (Dokcer)" : "(Produção)")}");


if (app.Evironment.IsDevelopment())
{
    app.Logger.LoginInformation("📖 Swagger disponível em: http://localhost:5000/");
}
